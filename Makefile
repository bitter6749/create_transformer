...
OBJS_C = $(patsubst $(SRC_DIR)/%.c, $(BUILD_DIR)/%.obj, $(SRCS_C))
...
# 1. GPU用コードのコンパイル
$(BUILD_DIR)/ops_cuda.obj: src/ops_cuda.cu | make_dirs
	@cmd /c "$(VS_ENV) && $(NVCC) $(CFLAGS) -c $< -o $@"

# 2. すべての .c ファイルを nvcc でコンパイル (MinGWの方言を排除)
$(BUILD_DIR)/%.obj: $(SRC_DIR)/%.c | make_dirs
	@cmd /c "$(VS_ENV) && $(CC) $(CFLAGS) -c $< -o $@"

# 3. 最終リンク（すべて同じ Microsoft 規格なので 100% 綺麗に合体します）
$(TARGET_TRAIN): $(OBJS_C) $(BUILD_DIR)/ops_cuda.obj $(APPS_DIR)/train.c
	@cmd /c "$(VS_ENV) && $(NVCC) $(CFLAGS) $(OBJS_C) $(BUILD_DIR)/ops_cuda.obj $(APPS_DIR)/train.c -o $@"
	@echo ">>> リンク完了: $(TARGET_TRAIN)"

train: $(TARGET_TRAIN)
	./$(TARGET_TRAIN)

$(TARGET_GEN): $(OBJS_C) $(BUILD_DIR)/ops_cuda.obj $(APPS_DIR)/generate.c
	@cmd /c "$(VS_ENV) && $(NVCC) $(CFLAGS) $(OBJS_C) $(BUILD_DIR)/ops_cuda.obj $(APPS_DIR)/generate.c -o $@"
