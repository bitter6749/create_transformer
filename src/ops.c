...
void forward_embedding(
  ...
  cudaMalloc((void **)&d_input_ids, seq_len * sizeof(int));
  cudaMemcpy(d_input_ids, input_ids, seq_len * sizeof(int), cudaMemcpyHostToDevice);
  
  forward_embedding_gpu(d_input_ids, embedding_table->device_data, X->device_data, seq_len, embed_dim);
...
void apply_positional_encoding(Matrix *X) {
  ...
  apply_positional_encoding_gpu(X->device_data, X->rows, X->cols);
}
...
void forward_layernom(
  ...
  forward_layernorm_gpu(X->device_data, gamma->device_data, beta->device_data, Out->device_data, X->rows, X->cols);
}
...
void softmax_row(Matrix *m, int row) {
  ...
  softmax_row_gpu(m->device_data, row, m->cols);
}
...
void compute_bias_gradient(const Matrix *dOut, Matrix *dBias) {
  ...
  //   dBias->data[i] = sum;
  // }
  compute_bias_gradient_gpu(dOut->device_data, dBias->device_data, dOut->rows, dOut->cols);
}
...
void copy_matrix(const Matrix *src, Matrix *dest) {
  ...
  copy_matrix_gpu(src->device_data, dest->device_data, total_elements);
}
...
void backward_embedding (
  ...
  backward_embedding_gpu(dX->device_data, d_input_ids, embedding_table->device_data, seq_len, embed_dim);
...
void backward_layernorm(
  ...
  backward_layernorm_gpu(dOut->device_data, X->device_data, gamma->device_data, dX->device_data, dgamma->device_data, dbeta->device_data, X->rows, X->cols);
}
...
void backward_softmax(const Matrix *dA, const Matrix *A, Matrix *dS) {
  ...
  backward_softmax_gpu(dA->device_data, A->device_data, dS->device_data, A->rows, A->cols);
}
...
void backward_relu(const Matrix *dOut, const Matrix *Out, Matrix *dIn) {
  ...
  backward_relu_gpu(dOut->device_data, Out->device_data, dIn->device_data, size);
}

void mat_compute_cross_entropy_grad(const Matrix *prob, Matrix *d_out, int target_id) {
  compute_cross_entropy_grad_gpu(prob->device_data, d_out->device_data, target_id, prob->cols);
}

void mat_write_row(const Matrix *src, Matrix *dest, int target_row) {
  write_row_gpu(src->device_data, dest->device_data, target_row, dest->cols);
}
